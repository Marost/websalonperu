...
mysql_query("INSERT INTO ".$tabla_suf."_fotos(url, titulo, contenido, fecha, hora, usuario) VALUES('$url', '$titulo', '$contenido', '$fecha', '$hora', '$usuario');", $conexion);

$rst_chica16=mysql_query("SELECT * FROM ".$tabla_suf."_fotos WHERE id>0 ORDER BY id DESC", $conexion);
...
while($_POST['flash_uploader_'.$cont.'_tmpname']<>""){
	...
	$thumb{$cont}->adaptiveResize(560,340);
	...
	mysql_query("INSERT INTO ".$tabla_suf."_fotos_slide(imagen, orden, noticia, carpeta, fecha, hora, usuario) VALUES ('$imagen', $cont, $idchica16, '$carpeta', '$fecha', '$hora', '$usuario')",$conexion);
	$cont++;
}
