...
function guardarArchivo($carpeta,$archivo){
	...
	{ 
		...
		$nombrese=codigoAleatorio(30, false, true, false);
...
function tipoVideo($tipo, $carpeta_video, $video, $imagen, $carpeta_imagen, $identificador, $ancho, $alto, $web){
	if($tipo=='dailymotion'){
		...
		$codigo='<object width="'.$ancho.'" height="'.$alto.'"><param name="movie" value="http://www.youtube.com/v/'.$video.'?fs=1&amp;hl=es_ES&amp;rel=0"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><embed src="http://www.youtube.com/v/'.$video.'?fs=1&amp;hl=es_ES&amp;rel=0" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="'.$ancho.'" height="'.$alto.'"></embed></object>';
	}elseif($tipo=='flv'){
		$codigo='<div class="player" 
					...
					<img src="'.$web.'imagenes/play_large.png" alt="Play this video" class="padding_0" />					
				</div>
